...
func exchangeToken(clientID, clientSecret, tokenURL, subjectToken, audience, scopes string, spireEnabled bool, jwtSvid, actorToken string) (string, error) {
	...
	if clientID != "" && hasCredentials && tokenURL != "" && targetAudience != "" && targetScopes != "" {
		...
		if authHeader != "" {
			...
			if subjectToken != authHeader {
				newToken, err := exchangeToken(clientID, clientSecret, tokenURL, subjectToken, targetAudience, targetScopes, spireEnabled, jwtSvid, actorToken)
				if err == nil {
